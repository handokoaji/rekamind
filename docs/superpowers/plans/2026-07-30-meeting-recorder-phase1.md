...
def test_meeting_with_related_rows_round_trips():
    async def scenario():
        ...
        async with session_factory() as session:
            loaded = await session.get(Meeting, await scenario_id)
        return loaded
...
Fix the test's `scenario_id` bug from Step 1 (it referenced a name that doesn't
exist) while implementing — rewrite `test_meeting_with_related_rows_round_trips`
to return the id directly from `scenario()`:

```python
# tests/storage/test_models.py (replace the test body)
def test_meeting_with_related_rows_round_trips():
